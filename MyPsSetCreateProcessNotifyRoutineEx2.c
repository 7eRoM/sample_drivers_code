...
void ProcessNotifyRoutineEx2(PEPROCESS Process, HANDLE ProcessId, PPS_CREATE_NOTIFY_INFO CreateInfo)
{
	if (CreateInfo)
	{
		DbgPrintEx(
...
VOID Unload(IN PDRIVER_OBJECT DriverObject)
{
	...
	NTSTATUS ntstatus = PsSetCreateProcessNotifyRoutineEx2(NotifyType, ProcessNotifyRoutineEx2, TRUE);
	if (!NT_SUCCESS(ntstatus))
	{
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "[-] Error in removing routine: Unable to call PsSetCreateProcessNotifyRoutineEx2 ...\r\n");
	}
	else
	{
...
NTSTATUS DriverEntry(IN PDRIVER_OBJECT DriverObject, IN PUNICODE_STRING RegistryPath)
{
	...
	PSCREATEPROCESSNOTIFYTYPE NotifyType = PsCreateProcessNotifySubsystems;
	NTSTATUS ntstatus = PsSetCreateProcessNotifyRoutineEx2(NotifyType, ProcessNotifyRoutineEx2, FALSE);
	if (!NT_SUCCESS(ntstatus))
	{
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "[-] Error in adding routine: Unable to call PsSetCreateProcessNotifyRoutineEx2 ...\r\n");
		return ntstatus;
	}
